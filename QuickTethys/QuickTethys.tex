...
This information was not set correctly when the other tutorial was
developed, and contains incorrect settings. First, you'll notice that
there are two hydrophone elements set: there is only one on the
SoundTrap HF-300, so click on one of them, and delete it. Next, edit the
remaining hydrophone (Figure~\ref{fig-hydrophone}): Enter the type as
ST-HF300, the sensitivity as -172.5, and the x,y,z coordinates all as
zero.
